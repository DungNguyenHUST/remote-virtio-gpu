	       ...
	       uint8_t *src)
{
	...
	if (b->plugin_v1.ctx.zmq) {
		struct rvgpu_scanout *s = &b->plugin_v1.scanout[0];

		if (inpdev->revents[0] & POLLIN) {
			ssize_t ret =
			    s->plugin_v1.ops.rvgpu_recv(s, COMMAND, buf, len);
			if (ret > 0) {
				if (src)
					*src = 0;
				return ret;
...
struct input_device *input_device_init(struct rvgpu_backend *b)
{
	...
	g->mouse =
	    create_input_device(mouse, ARRAY_SIZE(mouse), "rvgpu_mouse", NULL);
	if (g->mouse == -1)
		goto err_free;
