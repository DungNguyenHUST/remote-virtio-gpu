...
void rvgpu_egl_set_swap_skip(bool set) { _swap_skip = set; }
static bool rvgpu_egl_get_swap_skip(void) { return _swap_skip; }
		    ...
		    bool vsync)
{
	...
	if (rvgpu_egl_get_swap_skip() == false) {
		...
		if (e->params == NULL) {
			...
			glClearColor(
			    ((e->params->clear_color >> 24) & 0xFF) / 255.0f,
		...
		if (s->virgl.tex_id != 0) {
			...
			glBlitFramebuffer((int)vbox->x, (int)y1,
					  (int)(vbox->x + vbox->w), (int)y2, 0,
					  0, (int)s->window.w, (int)s->window.h,
...
/** Call before polling */
size_t rvgpu_egl_prepare_events(struct rvgpu_egl_state *e, void *ev, bool zmq,
				size_t max)
{
	if (e->cb->prepare_events)
		return e->cb->prepare_events(e, ev, zmq, max);

	return 0;
}

/** Call after polling */
void rvgpu_egl_process_events(struct rvgpu_egl_state *e, void *ev, bool zmq,
			      size_t n)
{
	if (e->cb->process_events)
		e->cb->process_events(e, ev, zmq, n);
}
