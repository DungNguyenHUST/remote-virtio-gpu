				...
				uint32_t time, uint32_t axis, wl_fixed_t value)
{
	...
	struct rvgpu_input_event ev = {
	    EV_KEY,
	    (axis == WL_POINTER_AXIS_VERTICAL_SCROLL) ? REL_WHEEL : REL_HWHEEL,
	...
	if ((button == BTN_RIGHT) &&
		...
		for (unsigned int i = 0; i < VIRTIO_GPU_MAX_SCANOUTS; i++) {
			if (r->egl.scanouts[i].native) {
				...
				if (surface)
					wl_shell_surface_move(surface, r->seat, serial);
			}
		}
	}
				      ...
				      bool zmq, size_t max)
{
	...
	if (!zmq) {
		...
		zmq_pollitem_t *fds = (zmq_pollitem_t *)ev;

		fds->fd = fd;
		fds->socket = NULL;
		fds->events = to_zmq_ev(events);
	}
	return 1;
}
				    ...
				    struct rvgpu_scanout *s)
{
	...
	if (r->shell) {
		...
		n->ivi_surface =
		    ivi_application_surface_create(r->ivi_app, id, n->surface);
