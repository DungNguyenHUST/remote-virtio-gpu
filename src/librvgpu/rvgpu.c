...
int rvgpu_ctx_send(struct rvgpu_ctx *ctx, const void *buf, size_t len)
{
	...
	for (unsigned int i = 0; i < ctx_priv->cmd_count; i++) {
		...
		if (!sc_priv->activated)
			return -EBUSY;
		int rc = write(sc_priv->pipes[COMMAND].snd_pipe[PIPE_WRITE],
			       buf, len);
		if ((rc != len) && (errno != EAGAIN))
			return errno;
	}
	       ...
	       struct rvgpu_scanout_arguments args)
{
	...
	if (args.zmq.zmq_endpoint) {
		rc = init_zmq_scanout(ctx, scanout, sc_priv->args);
		if (rc) {
			perror("Failed to init PGM scanout");
			goto error;
		}
	} else {
					...
					enum reset_state state))
{
	...
	if (ctx_priv->args.pgm) {
		if (pthread_create(&ctx_priv->tid, NULL, thread_conn_zmq,
				   ctx)) {
			perror("ZeroMQ thread creation error");
			return -1;
		}
	} else {
		if (pthread_create(&ctx_priv->tid, NULL, thread_conn_tcp,
				   ctx)) {
