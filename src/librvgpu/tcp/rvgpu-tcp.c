...
void close_conn(struct vgpu_host *vhost)
{
	...
	} else if (vhost->zmq_pfd) {
		if (vhost->zmq_pfd->fd > 0) {
			close(vhost->zmq_pfd->fd);
			vhost->zmq_pfd->fd = -1;
			vhost->zmq_pfd->socket = NULL;
			vhost->zmq_pfd->events = 0;
			vhost->zmq_pfd->revents = 0;
		}
	}
...
void process_reset_backend(struct rvgpu_ctx *ctx, enum reset_state state)
{
	...
	if ((!ctx_priv->args.keep_on_render) && (ctx_priv->gpu_reset_cb))
		  ...
		  unsigned int host_count)
{
	...
	if ((ctx_priv->args.keep_on_render) && (ctx_priv->gpu_reset_cb))
		ctx_priv->gpu_reset_cb(ctx, GPU_RESET_TRUE);
		   ...
		   unsigned int *active_sessions, unsigned int count)
{
	...
	for (unsigned int i = 0; i < count; i++) {
		if (vhost[i]->pfd) {
			...
			struct zmq_pollitem_t *host_pfd = vhost[i]->zmq_pfd;

			if ((host_pfd->events & ZMQ_POLLOUT) &&
			    (host_pfd->fd > 0)) {
				disconnect(vhost, 0, ctx->res_count, i);
				ses_hung = true;
				*active_sessions -= 1;
			}
		}
		     ...
		     struct pollfd *pfd, struct poll_entries *p_entry)
{
	unsigned int pfd_count =
	    get_pointers(ctx, pfd, &p_entry->ses_timer, &p_entry->recon_timer,
			 &p_entry->cmd_host, &p_entry->cmd_pipe_in,
			 &p_entry->res_host, &p_entry->res_pipe_in);
...
void handle_resources(struct rvgpu_ctx *ctx, struct vgpu_host *vhost[],
		      struct poll_entries p_entry, int devnull,
		      unsigned int act_ses)
{
	...
	if (!act_ses) {
		...
		set_in(p_entry.res_host, ctx_priv->res_count);
		return;
	}

	ret = get_input_ev(p_entry.res_pipe_in, ctx_priv->res_count);
	if (ret)
		set_in_out(p_entry.res_host, ctx_priv->res_count);
	else
		set_in(p_entry.res_host, ctx_priv->res_count);

	int rc = handle_host_res(ctx, vhost, &p_entry, devnull);

	enable_input(p_entry.res_pipe_in, ctx_priv->res_count);
...
void handle_commands(struct rvgpu_ctx *ctx, struct vgpu_host *vhost[],
		     struct poll_entries p_entry, int devnull,
		     unsigned int act_ses)
{
	struct ctx_priv *ctx_priv = (struct ctx_priv *)ctx->priv;
	...
	if (!act_ses) {
		...
		enable_input(p_entry.cmd_pipe_in, ctx_priv->cmd_count);
		return;
	}
	ret = get_input_ev(p_entry.cmd_pipe_in, ctx_priv->cmd_count);
	if (ret == act_ses) {
		disable_input(p_entry.cmd_pipe_in, ctx_priv->cmd_count);
		set_in_out(p_entry.cmd_host, ctx_priv->cmd_count);
		if (!timer_set) {
	...
	ret = handle_host_comm(ctx, vhost, &p_entry, devnull);
	if (ret)
		enable_input(p_entry.cmd_pipe_in, ctx_priv->cmd_count);
...
void *thread_conn_tcp(void *arg)
{
	...
	while (!ctx_priv->interrupted) {
		...
		/* Handle commands */
		handle_commands(ctx, vhost, p_entry, devnull, act_ses);
		/* Handle resources */
		handle_resources(ctx, vhost, p_entry, devnull, act_ses);
	}
