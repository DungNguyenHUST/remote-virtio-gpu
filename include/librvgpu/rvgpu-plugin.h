...
struct rvgpu_ctx_arguments {
	...
	uint16_t conn_tmt_s;
	/* TCP session timeout, to detect hung sessions */
	uint16_t session_tmt_ms;
	/* Scanout reconnection interval */
	uint16_t reconn_intv_ms;
	/* Number of scanouts */
	uint16_t scanout_num;
	/* In case of lost connection with a single target
	 * keep rendering to the other targets
	 */
	bool keep_on_render;
	/* Use PGM protocol for communication with remote targets
	 */
	bool pgm;
};
